...
class SingleGoalGridState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        for neighbour in neighboring_grid_locs:
            newS = SingleGoalGridState(neighbour, self.goal, self.dist_from_start+1, self.use_heuristic, self.maze_neighbors)
            nbr_states.append(newS)
        return nbr_states
    ...
    def __lt__(self, other):
        return (self.dist_from_start + self.h) < (other.dist_from_start + other.h) or ((self.dist_from_start + self.h) == (other.dist_from_start + other.h) and self.tiebreak_idx < other.tiebreak_idx)
...
class GridState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        for neighbor in neighboring_locs:
            if neighbor in self.goal:
                # Create a new goal state by removing the neighbor from the current goal
                new_goal = tuple(loc for loc in self.goal if loc != neighbor)
                new_state = GridState(neighbor, new_goal, self.dist_from_start + 1, self.use_heuristic, self.maze_neighbors, self.mst_cache)
            else:
                new_state = GridState(neighbor, self.goal, self.dist_from_start + 1, self.use_heuristic, self.maze_neighbors, self.mst_cache)
    ...
    def __eq__(self, other):
        return (self.state + self.goal) == (other.state + self.goal)
    ...
    def compute_heuristic(self):
        ...
        if len(self.goal) == 1:
            mst = 0
        elif self.goal in self.mst_cache:
            mst = self.mst_cache[self.goal]
        else:
            mst = compute_mst_cost(self.goal, manhattan)
            self.mst_cache[self.goal] = mst
        
        min_dist = float('inf')  # Initialize to positive infinity

        for goal in self.goal:
            min_dist = min(min_dist, manhattan(self.state, goal))
