...
class Application:
    ...
    def execute(self):
        if self.args.early_check:
            ...
            # print(gold_results, test_results)
            #             # Find the row indices where mat1 and mat2 don't match
            # non_matching_rows = np.where(~np.all(gold_results == test_results, axis=1))[0]
            # print(non_matching_rows)

            # Get the first column of each array
            first_col_arr1 = gold_results[:, 2]
            first_col_arr2 = test_results[:, 2]

            # Find the indices where the first columns don't match
            non_matching_rows = np.where(first_col_arr1 != first_col_arr2)[0]

            # Print the rows where values don't match
            for row_index in non_matching_rows:
                print(f"Row_{row_index + 1}: gold_results[{row_index}] = {gold_results[row_index]}, test_results[{row_index}] = {test_results[row_index]}")
            print(f"Total mismatch ={len(non_matching_rows)}")
...
def main():
    ...
    parser.add_argument('--window', dest="window", type=int, default=1000,
