...
class EightPuzzleState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        for dx, dy in directions:
            ...
            if 0 <= nx < 3 and 0 <= ny < 3:  # check if within bounds
                new_state = [row.copy() for row in self.state]  # copy current state
                new_state[x][y], new_state[nx][ny] = new_state[nx][ny], new_state[x][y]  # swap zero with neighbor
                nbr_states.append(EightPuzzleState(new_state, self.goal, self.dist_from_start + 1, self.use_heuristic, (nx, ny)))
        return nbr_states
    ...
    def compute_heuristic(self):
        ...
        for i in range(3):
            for j in range(3):
                num = self.state[i][j]
                if num != 0:  # don't compute distance for zero
                    goal_x, goal_y = divmod(num, 3)  # divmod gives quotient and remainder
